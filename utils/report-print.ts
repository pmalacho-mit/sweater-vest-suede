import type { ReportInput } from "./report-html.ts";
import type { TestResult } from "./report-events.ts";
...
export const printReport = (
  ...
  const byBrowser = input.browsers.reduce<
    Record<string, ReportInput["browsers"]>
  >((acc, entry) => {
  ...
  for (const [browser, entries] of Object.entries(byBrowser)) {
    ...
    for (const entry of entries) {
      const label =
        entry.componentPath
          ?.replace(/^\/src\//, "")
          .replace(/\.test\.svelte$/, "") ?? entry.kind;
      ...
      if (failed > 0) {
        write(` ${red("FAIL")}  ${label}   ${dim(`(${total} tests, ${ms(totalMs)})`)}\n`);
        ...
        write(` ${green("PASS")}  ${label}   ${dim(`(${total} tests, ${ms(totalMs)})`)}\n`);
      }
    }
  }
